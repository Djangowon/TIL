...
## 플러시
* 영속성 컨텍스트의 변경내용을 데이터 베이스에 반영하는 것임(동기화)
* 쌓아뒀던 영속성 컨텍스트의 SQL 쿼리들이 데이터베이스에 날려주는 것
* 보통 데이터베이스 트랜잭션이 커밋될 때 플러시가 일어남
* 영속성 컨텍스트를 비우지 않음
* 트랜잭션이라는 작업 단위가 중요함-> 커밋 직전에만 동기화 하면 됨
...
> 플러시를 하면 1차 캐시를 다 지우는 것이 아님  
> 영속 컨텍스트에 있는 쓰기 지연 저장소의 업데이트 쿼리들이 쌓인 것과, 변경 감지가 일어나고, 등록, 수정, 삭제 쿼리 들 몽땅 바뀐 것들이 데이터베이스에 반영이 되는 과정임
...
* 위의 코드에서 persist를 했을 때 데이터베이스에 쿼리가 날라가는 시점은 트랜잭션 커밋한 시점이기 때문에, memberC를 저장할 때까지 날아간 쿼리는 없음
* 바로 다음에 데이터베이스 조회를 하면 아무것도 안 나올 것임(에러)
* 이런 것을 방지하고자 JPA 기본 모드가 JPQL을 실행할 때 바로 플러시를 호출함
